...
class OfferService {
  ...
  // ✅ NEW — Cart threshold offers (cart_offers table)
  Future<List<Map<String, dynamic>>> getCartOffers() async {
    try {
      final data = await _client
          .from('cart_offers')
          .select()
  ...
  // ✅ NEW — Addon items (addon_items table)
  Future<List<Map<String, dynamic>>> getAddonItems() async {
    try {
      final data = await _client
          .from('addon_items')
          .select()
